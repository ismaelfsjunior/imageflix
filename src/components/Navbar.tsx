    ...
    <nav className="navbar absolute top-0 left-0 grid grid-cols-2 items-center w-full p-8">
        ...
        <div className="justify-self-end flex justify-items-end items-center">
            <form className="relative">
                <input className="w-full bg-black border border-white rounded py-1 px-3 transition-all duration-300 ease-in-out opacity-0 hover:opacity-100 focus:opacity-100" type="text" placeholder="Título, gente e gêneros" />
                <button className="search absolute right-0 py-1 px-2" onClick={ e => e.preventDefault()}><FontAwesomeIcon icon={faSearch} /></button>
            </form>
            <div className="relative flex ml-4">
                ...
                <ul className={`absolute pl-2 mt-8 top-2 right-0 w-32 bg-black rounded py-1 transition-all duration-500 ease-in-out ${!isMenuOpen && 'opacity-0 invisible'}`}>
